...
spark = SparkSession.builder.appName("HW_7").getOrCreate()

sql.execute("""
drop table if exists spark.DZ_7""",con)
sql.execute("""
CREATE TABLE if not exists spark.`DZ_7` (
...
dfG = spark.read.format("com.crealytics.spark.excel")\
	...
	.option("usePlainNumberFormat","true")\
        ...
        .option("header", "true")\
        .format("excel")\
        .load("/home/anna/DZ_7.xlsx").limit(1000)\
...
df_combined.write.format("jdbc").option("url","jdbc:mysql://localhost:33061/spark?user=root&password=1")\
        .option("driver", "com.mysql.cj.jdbc.Driver").option("dbtable", "DZ_7")\
        .mode("append").save()
